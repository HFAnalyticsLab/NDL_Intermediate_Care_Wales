...
Purpose: Extracts long-term conditions from WLGP. Creates lookup tables flagging presence of long-term conditions in matched cohort using WLGP data.
...
DROP TABLE SAILW1658V.LB_LTC_CANCER_LKUP_RCT;
DROP TABLE SAILW1658V.LB_LTC_CKD_LKUP_RCT;
DROP TABLE SAILW1658V.LB_LTC_MEDCODE_LKUP_RCT;
DROP TABLE SAILW1658V.LB_LTC_PRODCODE_LKUP_RCT;
...
-- 1. CANCER
CREATE TABLE SAILW1658V.LB_LTC_CANCER_LKUP_RCT(
	ALF_PE BIGINT,
...
INSERT INTO SAILW1658V.LB_LTC_CANCER_LKUP_RCT
SELECT 
...
FROM
	...
	FROM
		...
		SELECT
			...
			CANEVENT.EVENT_CD
		FROM
			SAILW1658V.JP_MATCHED_COHORT_RCT RCT
...
-- 2. KIDNEY
CREATE TABLE SAILW1658V.LB_LTC_CKD_LKUP_RCT(
	ALF_PE BIGINT,
	REABLEMENT_START_DATE DATE,
	COND VARCHAR(50), 
	COND_DESC VARCHAR (500),
...
INSERT INTO SAILW1658V.LB_LTC_CKD_LKUP_RCT
SELECT 
	ALF_PE,
	REABLEMENT_START_DATE,
	COND,
...
FROM
	...
	FROM
		...
		SELECT
			...
			ROW_NUMBER() OVER(PARTITION BY RCT.ALF_PE ORDER BY RCT.ALF_PE, EVENT_DT DESC, "SEQUENCE" DESC) AS RN
		FROM
			SAILW1658V.JP_MATCHED_COHORT_RCT RCT
		...
		INNER JOIN (
			...
			RCT.ALF_PE = CKDEVENT.ALF_PE
		-- 2.4 Select records that occured before being identified as an unpaid carer
		WHERE CKDEVENT.EVENT_DT < RCT.REABLEMENT_START_DATE)
...
-- 3. ALL OTHER 'MEDCODES' 
CREATE TABLE SAILW1658V.LB_LTC_MEDCODE_LKUP_RCT(
	ALF_PE BIGINT,
	REABLEMENT_START_DATE DATE,
	COND VARCHAR(50),
	COND_DESC VARCHAR(500),
...
INSERT INTO SAILW1658V.LB_LTC_MEDCODE_LKUP_RCT
SELECT 
...
FROM 
	...
	INNER JOIN (
		...
		RCT.ALF_PE = MEDEVENT.ALF_PE
	-- 3.5 ...where event_dt between first_identified_date - read and first_identified_date - 1
	WHERE MEDEVENT.EVENT_DT BETWEEN RCT.REABLEMENT_START_DATE - MEDEVENT."READ" YEARS AND RCT.REABLEMENT_START_DATE - 1 DAYS);


-- 4. PRODCODES
CREATE TABLE SAILW1658V.LB_LTC_PRODCODE_LKUP_RCT(
	ALF_PE BIGINT,
	REABLEMENT_START_DATE DATE,
	COND VARCHAR(50), 
	COND_DESC VARCHAR(500),
...
INSERT INTO SAILW1658V.LB_LTC_PRODCODE_LKUP_RCT
SELECT 
	ALF_PE,
	REABLEMENT_START_DATE,
	COND,
	...
	FROM
		...
		INNER JOIN (
			...
			INNER JOIN (
				...
				ON PRODEVENT.ALF_PE = RCT.ALF_PE 
		WHERE
			-- 4.5 ...where condition is scz and readcode ever recorded...
			(COND = 'SCZ' AND PRODEVENT.EVENT_DT < RCT.REABLEMENT_START_DATE) OR
			-- 4.6 ...or condition not scz and event_dt between first_identified_date - 365 and first_identified_date - 1 
			(COND != 'SCZ' AND PRODEVENT.EVENT_DT BETWEEN RCT.REABLEMENT_START_DATE - 1 YEARS AND RCT.REABLEMENT_START_DATE - 1 DAYS))
	GROUP BY 
		ALF_PE,
		REABLEMENT_START_DATE,
		COND,
